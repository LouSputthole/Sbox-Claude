...
export function registerAuditTools(
  ...
  server.tool(
    "find_broken_references",
    "Scan every GameObject in the open scene for broken references: renderers with no Model assigned (missing_model), component properties pointing at DESTROYED GameObjects/Components (dead_gameobject_ref / dead_component_ref), and null component entries whose type no longer exists (missing_component). Returns { total, showing, truncated, objectsScanned, issues } — each issue has { id, name, component, kind, detail }. Fix missing models with assign_model; clear dead refs with set_property or set_component_reference. Read-only; safe any time. Results cap at `limit` (default 100, max 500) with truncated:true when more exist",
    {
      limit: z
        .number()
        .int()
