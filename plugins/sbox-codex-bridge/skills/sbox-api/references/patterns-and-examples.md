...
- [Example 1 — Health Component with Damage Events](#example-1--health-component-with-damage-events)
- [Example 2 — First-Person Character Controller](#example-2--first-person-character-controller)
- [Example 3 — Hitscan Weapon with Networked Effects](#example-3--hitscan-weapon-with-networked-effects)
- [Example 4 — Networked Game Manager & Player Spawning](#example-4--networked-game-manager--player-spawning)
- [Example 5 — Networked Player with Sync, RPCs, and Proxy Safety](#example-5--networked-player-with-sync-rpcs-and-proxy-safety)
- [Example 6 — Razor HUD Panel with Data Binding](#example-6--razor-hud-panel-with-data-binding)
- [Example 7 — Physics Grenade with Trigger Proximity & Explosion](#example-7--physics-grenade-with-trigger-proximity--explosion)
- [Example 8 — NavMeshAgent AI with State Machine](#example-8--navmeshagent-ai-with-state-machine)
- [Example 9 — Prefab Spawner with Pool-Friendly Lifecycle](#example-9--prefab-spawner-with-pool-friendly-lifecycle)
- [Example 10 — Trigger Zone (Pickup / Checkpoint)](#example-10--trigger-zone-pickup--checkpoint)
- [Quick-Reference Patterns](#quick-reference-patterns)
  - [Local-player query](#local-player-query)
  - ["Do this on the host only"](#do-this-on-the-host-only)
  - ["Do this on the owner only"](#do-this-on-the-owner-only)
  - ["Do this on everyone with authoritative data"](#do-this-on-everyone-with-authoritative-data)
  - [Schedule work after a delay](#schedule-work-after-a-delay)
  - [Trace from crosshair](#trace-from-crosshair)
  - [Find all players and iterate](#find-all-players-and-iterate)
  - [Broadcast an effect without syncing a GameObject](#broadcast-an-effect-without-syncing-a-gameobject)
  - [Guard against destroyed references](#guard-against-destroyed-references)
  - [Tag-based collision filter (cheaper than type checks)](#tag-based-collision-filter-cheaper-than-type-checks)
  - [One-shot sound](#one-shot-sound)
...
public sealed class HealthPickup : Component, Component.ITriggerListener
{
    ...
    public void OnTriggerEnter( Collider other )
    {
        ...
        // Health.Current must itself be host-authoritative (for example,
        // [Sync(SyncFlags.FromHost)]) and only mutated through host-validated code.
    ...
    void OnAvailableChanged( bool oldValue, bool newValue )
    {
        ApplyAvailability();
    }
...
Key points:
- Only the host handles the pickup and mutates health or availability. Make the health field host-authoritative too; a host gate around this trigger does not repair an owner-authored health model elsewhere.
- `[Sync(SyncFlags.FromHost)]` exposes availability without letting a client author it. The `[Change]` callback derives persistent visibility from that state, while `OnStart` applies it for late joiners. RPCs remain transient effects only.
