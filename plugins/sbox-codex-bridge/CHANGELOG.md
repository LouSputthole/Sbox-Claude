...
- Bundled the enabled native s&box MCP endpoint and the pinned, opt-in `sbox-mcp-server@2.3.0` lifeline.
...
**Recommended immutable release tag:** `codex-v2.3.0`. The repository tag `v2.1.0` predates the Codex package; Codex releases use the separate `codex-v*` tag series.
