...
- [The one idiom that makes it work: host-authoritative money](#the-one-idiom-that-makes-it-work-host-authoritative-money)
- [The system stack to compose](#the-system-stack-to-compose)
- [Build order](#build-order)
- [How the real game does each piece](#how-the-real-game-does-each-piece)
  - [Interaction — Component.IPressable with a live tooltip](#interaction--componentipressable-with-a-live-tooltip)
  - [Content as assets — GameResource definitions](#content-as-assets--gameresource-definitions)
  - [Weighted loot table — the gacha roller](#weighted-loot-table--the-gacha-roller)
  - [Idle income — the MoneyPrinter (and the bucketed-[Sync] perf trick)](#idle-income--the-moneyprinter-and-the-bucketed-sync-perf-trick)
  - [Persistent world-entity identity — durable IDs for placed objects](#persistent-world-entity-identity--durable-ids-for-placed-objects)
  - [World save — scene-diff against a baseline](#world-save--scene-diff-against-a-baseline)
  - [Per-player persistence — versioned, with a migration ladder](#per-player-persistence--versioned-with-a-migration-ladder)
  - [Chat slash-command router](#chat-slash-command-router)
- [Standout engineering discipline](#standout-engineering-discipline)
...
- [Corpus refresh (2026): more reference implementations](#corpus-refresh-2026-more-reference-implementations)
  - [Alternative: EventBus money (non-[Sync]) vs [Sync(FromHost)]](#alternative-eventbus-money-non-sync-vs-syncfromhost)
  - [Ownable/lockable doors with linked groups and job-access tiers](#ownablelockable-doors-with-linked-groups-and-job-access-tiers)
  - [Rpc.FilterInclude for targeted (whisper / proximity / team) RPCs](#rpcfilterinclude-for-targeted-whisper--proximity--team-rpcs)
  - [Time-boxed vote flow (snapshot electorate → filtered UI → async timeout → resolve)](#time-boxed-vote-flow-snapshot-electorate--filtered-ui--async-timeout--resolve)
  - [Dependency-ordered module kernel with cycle detection and fail isolation](#dependency-ordered-module-kernel-with-cycle-detection-and-fail-isolation)
  - [Host-as-superadmin shortcut + Connection.Id-keyed transient admin state](#host-as-superadmin-shortcut--connectionid-keyed-transient-admin-state)
  - [Network interest culling (Component.INetworkVisible) — artisan.darkrpog](#network-interest-culling-componentinetworkvisible--artisandarkrpog)
  - [Razor panel performance — BuildHash + PerFramePanelCache + distance-gated OnUpdate](#razor-panel-performance--buildhash--perframepanelcache--distance-gated-onupdate)
  - [World-panel kiosk without collider alignment (WorldInput.Hovered)](#world-panel-kiosk-without-collider-alignment-worldinputhovered)
  - [Off-thread coalescing persistence writer — artisan.darkrpog](#off-thread-coalescing-persistence-writer--artisandarkrpog)
  - [Anti-farm XP on repeatable actions — artisan.darkrpog](#anti-farm-xp-on-repeatable-actions--artisandarkrpog)
  - [Per-player-cap vendor with live-object tracking — lowkeynetworks.newrp](#per-player-cap-vendor-with-live-object-tracking--lowkeynetworksnewrp)
- [Read these games](#read-these-games)
<!-- reference-toc:end -->

How to build a persistent, host-authoritative roleplay sandbox (DarkRP-style) in modern s&box (GameObject/Component/Scene), informed by `artisan.darkrpog` — a 2,517-file production DarkRP framework. Most of its *content* (drugs, casino, jobs) is genre-specific; its *plumbing* (persistence, host-authority, data-as-assets, interaction) also applies to other persistent multiplayer games.
...
This engine-free algorithm supports drop tables, shop restocks, and random spawns. It returns a `.Clone()` so callers cannot mutate the catalog.
...
A printer accumulates `StoredMoney` on a cadence, has integer upgrade levels (Speed/Yield/Capacity), and a Heat mechanic that can catch fire. Its key networking lesson is **bucketed `[Sync]` writes**: for a value that changes every fixed tick, accumulate a local delta and only write the `[Sync]` property when it crosses a threshold — otherwise you ship a network packet to every viewer 60×/sec for a fractional change.
...
## Standout engineering discipline
...
**Anti-pattern in newrp:** `Math.Max(0, value)` (System.Math) appears in the source but will not compile in s&box's sandbox — use `MathX.Max(0, value)` in the s&box implementation.
...
artisan's ATM screen demonstrates three Razor performance rules that apply to any world-facing panel reading live game state.
...
- `artisan.darkrpog` — production-grade framework: off-thread persistence, network interest culling, frame-budget queues, Razor panel perf, world-panel kiosk, anti-farm XP, 45 GameResource asset types, data-driven economy ROI simulator.
- `lowkeynetworks.newrp` — framework architecture: dependency-ordered module kernel, `Rpc.FilterInclude` proximity/vote targeting, ownable linked-door groups, EventBus money (non-`[Sync]` alternative), per-player-cap vendor with live-object tracking, `[ConCmd]`→`[Rpc.Host]` chat intercept.

Inspect both projects through the public links in [SOURCE-PROVENANCE.md](../SOURCE-PROVENANCE.md).
