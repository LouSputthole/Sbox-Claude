...
- [The one idiom that makes co-op work](#the-one-idiom-that-makes-co-op-work)
- [Build order](#build-order)
- [How the real games do each piece](#how-the-real-games-do-each-piece)
  - [Customer AI — activity-queue FSM on a NavMeshAgent](#customer-ai--activity-queue-fsm-on-a-navmeshagent)
  - [Spawner — scaling caps + daily rush](#spawner--scaling-caps--daily-rush)
  - [Supplier stock — day-seeded determinism](#supplier-stock--day-seeded-determinism)
  - [Shelf placement — grid from model bounds](#shelf-placement--grid-from-model-bounds)
  - [Interaction + carry](#interaction--carry)
  - [Cash register — checkout queue](#cash-register--checkout-queue)
  - [Worker AI — global job board (optional automation)](#worker-ai--global-job-board-optional-automation)
  - [Save/load — scan + factory respawn](#saveload--scan--factory-respawn)
- [Judgment variant (donerkiosk)](#judgment-variant-donerkiosk)
...
- [Corpus refresh (2026): more reference implementations](#corpus-refresh-2026-more-reference-implementations)
  - [Charge → act → refund-on-fail rollback (thefancylads.restaurantdev)](#charge--act--refund-on-fail-rollback-thefancyladsrestaurantdev)
  - [Sync occupancy bits, not object graphs (thefancylads.restaurantdev)](#sync-occupancy-bits-not-object-graphs-thefancyladsrestaurantdev)
  - [Struct-of-Arrays (SoA) for replicating lists of structs (emg.everythingmustgo)](#struct-of-arrays-soa-for-replicating-lists-of-structs-emgeverythingmustgo)
  - [Authority-forked read properties (emg.everythingmustgo)](#authority-forked-read-properties-emgeverythingmustgo)
  - [Queue hash for cheap Razor re-renders (emg.everythingmustgo)](#queue-hash-for-cheap-razor-re-renders-emgeverythingmustgo)
  - [Three-spend-path currency pattern (enifun.shopmanager)](#three-spend-path-currency-pattern-enifunshopmanager)
  - [Price-elasticity demand curve via control-point array (enifun.shopmanager)](#price-elasticity-demand-curve-via-control-point-array-enifunshopmanager)
  - [Data-driven modifier engine via EffectsManager (thefancylads.restaurantdev)](#data-driven-modifier-engine-via-effectsmanager-thefancyladsrestaurantdev)
  - [Dynamic pricing from recipe complexity (thefancylads.restaurantdev)](#dynamic-pricing-from-recipe-complexity-thefancyladsrestaurantdev)
  - [INetworkVisible distance culling for crowded scenes (thefancylads.restaurantdev)](#inetworkvisible-distance-culling-for-crowded-scenes-thefancyladsrestaurantdev)
  - [Data-driven catalogs as plain static C# (emg.everythingmustgo)](#data-driven-catalogs-as-plain-static-c-emgeverythingmustgo)
  - [Modal cursor UI borrow/restore (emg.everythingmustgo)](#modal-cursor-ui-borrowrestore-emgeverythingmustgo)
  - [Per-player-plot foreign-object eviction (thefancylads.restaurantdev)](#per-player-plot-foreign-object-eviction-thefancyladsrestaurantdev)
  - [Versioned JSON-node migration chain (enifun.shopmanager)](#versioned-json-node-migration-chain-enifunshopmanager)
  - [Clothing blacklist sanitizer for Dresser.Randomize() (luckygaming.donerkiosk)](#clothing-blacklist-sanitizer-for-dresserrandomize-luckygamingdonerkiosk)
  - [Hotload-safe singleton via IHotloadManaged (luckygaming.donerkiosk)](#hotload-safe-singleton-via-ihotloadmanaged-luckygamingdonerkiosk)
...
SHOP MANAGER distinguishes three semantically distinct spend paths with different authority and failure semantics:

```text
currency service operations:
  add-money(amount, reason): reject non-positive income, then credit and record the reason
  try-spend(amount, reason): reject invalid or unaffordable requests; otherwise debit and return success
  force-spend(amount, reason): apply an authorized system deduction even when it creates debt
  can-afford(amount): pure read used by presentation and preflight checks
```
...
```text
for every affected gameplay formula, request the owner's aggregate modifier by effect type:
  advance cooking progress using the cooking-method speed modifier
  scale per-step customer spawn probability by the spawn modifier
  scale patience duration by the patience modifier
  scale computed meal value by the value modifier
```

`UpgradeEffect` is a `struct { EffectType Type; EffectMode Mode(Add/Multiply/Flag); float ValuePerRank; }`. Multiplicative effects return `1 + sum`; additive return `sum`. `GetDescription(rank)` generates tooltip text from the same data, so UI never drifts from the math. The separation works for any upgrade or prestige tree.
...
- `emg.everything_must_go` — pawn-shop tycoon; SoA net-sync, authority-forked reads, tag-synergy economy, job-queue workers, navmesh-aware checkout line, ghost placement
- `enifun.shop_manager` — grocery supermarket; price-elasticity demand curve, phone orders, 4-tier permissions, versioned migration chain, day-night clock, late-joiner hydration
- `thefancylads.restaurant_dev` — restaurant tycoon; charge→act→refund rollback, occupancy-bit sync, EffectsManager modifier engine, `INetworkVisible` cull, `#if SERVER` compile-split, REST backend persistence
- `luckygaming.doner_kiosk` — anomaly-detection horror shopkeeper; judgment via FSM exit branch, observation-gated reveals, TTS voice, clothing blacklist, hotload-safe singleton

Inspect the cited projects through the public links in [SOURCE-PROVENANCE.md](../SOURCE-PROVENANCE.md).
