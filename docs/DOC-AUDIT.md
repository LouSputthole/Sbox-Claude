...
Current working-source numbers all non-historical docs should agree on (v2.1.0 plus the Unreleased placement, geometry, and camera batch):
**273 native tools / 28 toolsets / 57 read-only / 7 lifeline / 286 total / 278 handlers.**
(The published v2.0.0 release was 232/245/237 —
docs that explicitly label a number as a historical v2.0.0 snapshot are
...
| `CLAUDE.md` | current | contributor / internal | Architecture, verified s&box APIs, lessons learned. | **Rewritten for v2** (native transport as primary, file IPC as legacy fallback). Header carries the current source counts (273/286/278) and distinguishes them from the published v2.0.0 (245/237). |
...
| `TROUBLESHOOTING.md` (root) | current | public | Legacy **file-IPC** transport troubleshooting. | Correctly scoped as the legacy-fallback doc; `docs/TROUBLESHOOTING.md` links to it as such. Accurate for the compatibility fallback retained in current source. |
| `TESTING.md` | current | contributor / internal | Test plan (v2, native MCP). | **Rewritten for v2**: automated gates + native-server smoke path (`search_tools` / `call_tool` / inline images); the exhaustive v1 test matrix retired while the compatibility transport remains covered separately. Current smoke expectations use 273 native / 286 total / 278 handlers and `handlerCount` = 278. |
...
| `docs/TOOLSETS.md` | generated | public / agent | The authoritative tool inventory (273 tools / 28 toolsets / 57 read-only). | **Do not hand-edit** (emitted by `scripts/emit-mcp-wrappers.mjs`). Header numbers are the source of truth; regenerated for v2.1.0 (incl. the cinematic wave). |
...
| `docs/TROUBLESHOOTING.md` | **updated** | public | v2 native-transport failure modes (port 7269, stale HTTP.sys, missing tools, recompile, play-mode, modal stalls). | Added RELAUNCH/FAQ/INSTALL to the companion-docs list. v2-accurate; current source tool count is 273. |
...
| `docs/asset-library-listing.md` | **updated** | public / marketing | s&box Asset Library store listing copy. | Counts refreshed to the current source totals (273/286/278). The per-toolset "Full tool list" remains the v2.0.0 snapshot (noted inline; TOOLSETS.md is authoritative) — refresh it when the release publishes. |
...
3. ~~**`TESTING.md`**~~ — ✅ rewritten: native-server gates + smoke path; v1 exhaustive test matrix retired; fallback transport retained.
4. ~~**`CHANGELOG.md` `[2.0.0]`**~~ — ✅ carries a final-totals reconciliation note above the
   wave-snapshot counts.
5. ~~**`docs/asset-library-listing.md`**~~ — ✅ current headlines refreshed; its explicitly labeled v2.0.0 list remains historical.

Still open:

6. **`NEW_TOOLS_HANDOFF.md`** — self-deprecated; consider deleting.
7. **Minor:** regenerate `docs/graph/GRAPH_REPORT.md` at release per CLAUDE.md; the asset-listing per-toolset block remains explicitly labeled as a v2.0.0 snapshot.
