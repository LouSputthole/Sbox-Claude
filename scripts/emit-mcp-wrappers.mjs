...
// Hand-written in BridgeScreenshotTools.cs (inline PNGs + camera workflows).
const HAND_WRITTEN = new Set([
  "take_screenshot", "screenshot_from", "capture_view", "screenshot_orbit",
  "save_camera_bookmark", "list_camera_bookmarks", "delete_camera_bookmark",
  "capture_camera_set", "capture_topdown",
]);
...
const TOOL_TOOLSET = {
  ...
  find_objects_near: "bridge_gameobject",
...
const TOOLSET_META = {
  ...
  bridge_networking: {
    ...
    description:
      "Multiplayer setup, local multi-instance testing, and codegen: network helpers, lobby managers, networked players, [Sync] properties, RPCs (broadcast/host/targeted), ownership, spawning, and host-migration recovery.",
  },
  bridge_npc: {
...
const RO_EXTRA = new Set([
  ...
  "playtest_status", "drive_player_status", "multiplayer_test_status", "validate_project", "checkpoint_scene",
]);
...
doc.push(`> ${total + HAND_WRITTEN.size} tools across ${toolsets.size + 1} toolsets (${roCount + 2} read-only).`);
doc.push(``);
...
doc.push(`| \`bridge_screenshot\` | 9 | 2 | Inline-PNG screenshots plus session camera bookmarks, ordered comparison sets, and deterministic top-down capture (hand-written). |`);
doc.push(``);
for (const [ts, tools] of [...toolsets.entries()].sort()) {
  doc.push(`## ${ts}`);
  doc.push(``);
...
doc.push(`image blocks with structured manifests; bookmarks live only for the editor session.`);
doc.push(``);
doc.push(`| Tool | Mode | Description |`);
doc.push(`|---|---|---|`);
...
doc.push(`| \`save_camera_bookmark\` | **mutate** | Save or replace a named session camera pose from a target, explicit pose, or current main camera. |`);
doc.push(`| \`list_camera_bookmarks\` | read | List session camera bookmarks in stable name order. |`);
doc.push(`| \`delete_camera_bookmark\` | **mutate** | Delete a session camera bookmark and its comparison baselines. |`);
doc.push(`| \`capture_camera_set\` | **mutate** | Capture 1-8 bookmarks in requested order with labels and optional previous-capture RGBA diff metrics. |`);
doc.push(`| \`capture_topdown\` | **mutate** | Capture a deterministic orthographic top-down PNG with ground-plane scale metadata. |`);
doc.push(``);
